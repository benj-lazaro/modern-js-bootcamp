...
// Promisify the crypto.scrypt() method
...
class UsersRepository {
  ...
  async create(attrs) {
    ...
    // Generate an 8-byte hexadecimal salt value
    const salt = crypto.randomBytes(8).toString("hex");

    // Generate a hash password based on plaintext password, salt value & key length of 64
    const buffer = await scrypt(attrs.password, salt, 64);

    // Read the current content (an array) of users.json
    const records = await this.getAll();

    // Open attrs object, replace plaintext password with the generated hashed password
    // Along with the salt value, separated by a "." character
    const record = {
      ...attrs,
      password: `${buffer.toString("hex")}.${salt}`,
    };

    // Push the updated record into the records array
    records.push(record);
  ...
  // Compare user supplied password from saved the salted+hashed password from repository
  async comparePasswords(saved, supplied) {
    // Deconstruct the saved salted+hashed password into separate array elements
    const [hashed, salt] = saved.split(".");

    // Hash the supplied password with the stored salt value and a key value of 64
    const hashedSuppliedBuffer = await scrypt(supplied, salt, 64);

    // Compare the two passwords & return the boolean result
