...
const test = async () => {
  ...
  // Get one record by
  const recordFoundById = await repo.getOneBy({ id: "3852039d" });
  console.log("Record found by ID");
  console.log(recordFoundById);

  const recordMultipleFilters = await repo.getOneBy({
    email: "qwerty@test.com",
    password: "password1234567890",
  });
  console.log("Record found using multiple filters");
  console.log(recordMultipleFilters);

  const nonexistentRecord = await repo.getOneBy({
    email: "anonymous@test.com",
  });
  console.log("Non-existent record");
  console.log(nonexistentRecord);
};
